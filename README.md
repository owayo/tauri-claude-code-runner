...
GitHub Actionsでビルドされたアプリケーションは署名されていないため、初回起動時に「壊れているため開けません」というエラーが表示される場合があります。以下の手順で開いてください：

1. **DMGファイルをダブルクリックしてマウント**
2. **アプリケーションをApplicationsフォルダーにドラッグ**
3. **ダブルクリックではなく、右クリック（またはControlキーを押しながらクリック）**
4. **コンテキストメニューから「開く」を選択**
5. **警告ダイアログで「開く」をクリック**

一度この手順で開いた後は、通常通りダブルクリックで起動できるようになります。
